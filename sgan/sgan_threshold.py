...
class PoolHiddenNet(nn.Module):
    ...
    def forward(self, h_states, seq_start_end, end_pos):
        ...
        npeds = seq_start_end[-1][-1].item()
        h_states_view = h_states.view(npeds, self.h_dim)
        pool_h = []
        for _, (start, end) in enumerate(seq_start_end):
            start = start.item()
            end = end.item()
            num_ped = end - start
            curr_hidden = h_states_view[start:end]
            d_curr = curr_hidden.detach().cpu().numpy()
...
class TrajectoryGenerator(nn.Module):
    def __init__(
        ...
        pool_every_timestep=True, dropout=0.0, bottleneck_dim=1024,
        activation='relu', batch_norm=True, neighborhood_size=2.0, grid_size=8, threshold=0.5
    ):
